...
<svelte:head>
	<title>Followers of {data.profileUser.username} - Oshibana</title>
</svelte:head>
...
<div class="mx-auto max-w-7xl px-4 py-12">
	<div class="mb-8 flex items-center gap-4">
		...
		<div>
			<h1 class="text-3xl font-bold tracking-tight">Followers</h1>
			<p class="mt-2 text-muted-foreground">
				Users following <span class="font-medium text-foreground">{data.profileUser.username}</span>
			</p>
		</div>
	</div>
	...
	{#if data.followers.length === 0}
		...
		>
			...
			<h3 class="mt-4 text-lg font-semibold">No followers yet</h3>
			<p class="mt-2 text-sm text-muted-foreground">
				{data.user?.id === data.profileUser.id
					? "You don't have any followers yet."
					: `${data.profileUser.username} doesn't have any followers yet.`}
			</p>
		</div>
	{:else}
