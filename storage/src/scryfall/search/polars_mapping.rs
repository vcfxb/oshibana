    ...
    for c in cols.iter() {
        let mapped = match c {
            SearchViewColumn::Name => when(col("printed_name").is_null())
                .then(col("name"))
                .otherwise(format_str("{} ({})", &[col("printed_name"), col("name")]).expect("name formatting"))
                .alias("Name"),


            SearchViewColumn::Type => when(col("printed_type_line").is_null())
                .then(col("type_line"))
                .otherwise(format_str("{} ({})", &[col("printed_type_line"), col("type_line")]).expect("type line formatting"))
                .alias("Type"),
...
pub fn apply_preprocessing(cards_lf: LazyFrame) -> LazyFrame {
    ...
    cards_lf
        .with_columns([
            col("mana_cost").fill_null(format_str("{} // {}", &[
                col("card_faces").list().first().struct_().field_by_name("mana_cost"),
                col("card_faces").list().get(lit(1), true).struct_().field_by_name("mana_cost")
