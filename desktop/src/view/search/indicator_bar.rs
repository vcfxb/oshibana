...
use egui::{pos2, vec2, Color32, CornerRadius, Pos2, Rect, Ui};
...
pub fn indicator_bar(ui: &mut Ui, search_rect: Rect, active: bool) {
    ...
    let rect = Rect {
        min: Pos2 { y: search_rect.max.y + offset, x: search_rect.min.x },
        max: Pos2 { y: search_rect.max.y + height + offset, x: search_rect.max.x }
    };
    ...
    match active {
        ...
        true => {
            ...
            let bar_rect = Rect::from_min_size(
                pos2(current_x, rect.top()),
                vec2(bar_width, rect.height()),
            );

            ui.painter().rect_filled(rect, rounding, ui.visuals().extreme_bg_color);
            ui.painter().rect_filled(bar_rect, rounding, Color32::LIGHT_BLUE);
        }
    }
}
